...
fn_help() {
  ...
  echo "   * Example #1: Discover process pids with exactly names \"trivial-rewrite\" and \"qmgr\" that are owned by postfix and discover process pids with \"zabbix\" on its name and \"/usr/bin\" regexp on its entire command line -> ${0} '{\"postfix\":{\"exactly\":[\"trivial-rewrite\",\"qmgr\"]},\"root\":{\"regexp\":[\"zabbix\",\"ssh\"],\"cmd\":[\"/usr/bin\"]}}' discovery"
  echo "   * Example #2: ${0} '{\"postfix\":{\"exactly\":[\"trivial-rewrite\",\"qmgr\"]},\"root\":{\"regexp\":[\"zabbix\",\"ssh\"],\"cmd\":[\"/bin/sh\"]}}' discovery"
